...
export function Numbers({ S }: { S: Scope }) {
  ...
  React.useEffect(() => {
    const f = (e: Event) => { if ((e as CustomEvent<string>).detail === 'numeros') setOpenSim(true) }
    window.addEventListener('section-open', f); return () => window.removeEventListener('section-open', f)
  }, [])
  ...
  return (
    <Section id="numeros" n="06" eyebrow="Punto de equilibrio" title="¿Cierran los números?"
      ...
      <Collapsible open={openSim} onOpenChange={setOpenSim}>
        <CollapsibleTrigger className="inline-flex items-center gap-1.5 rounded-lg border bg-white px-3 py-2 text-sm font-medium text-foreground shadow-xs transition-colors hover:bg-muted">
          <SlidersHorizontal className="size-4 text-primary" />{openSim ? 'Ocultar simulador' : 'Ver simulador'}<ChevronDown className={cn('size-4 transition-transform duration-200', openSim && 'rotate-180')} />
        </CollapsibleTrigger>
